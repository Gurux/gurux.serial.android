...
class GXCP21xx extends GXChipset {
    ...
    private static final int ENABLE_INTERFACE = 0x0;
    ...
    public boolean open(GXSerial serial, UsbDeviceConnection connection, byte[] rawDescriptors) throws IOException {
        ...
        ret = connection.controlTransfer(HOST_TO_DEVICE_REQUEST_TYPE, SET_BAUDRATE,
                ...
                serial.getWriteTimeout());
        if (ret != 4) {
            throw new IOException("Failed to set baudrate: " + ret);
        }

        int value = (serial.getDataBits() << 8);
        value |= (serial.getParity().ordinal() << 4);
        value |= serial.getStopBits().ordinal();
